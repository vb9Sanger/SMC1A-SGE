...
default -- override with --groups):
  ...
  - pairwise odds ratios (Haldane-Anscombe corrected where a cell is 0) with
    95% CIs and Fisher's exact p, for every pair of groups supplied
  - each OR's ClinGen SVI OddsPath evidence-strength tier (Very Strong /
    Strong / Moderate / Supporting / none), using the Brnich et al. 2019
    default thresholds (350 / 18.7 / 4.33 / 2.08)
...
def oddspath_tier(or_value):
    for threshold, label in ODDSPATH_TIERS:
        if or_value >= threshold:
            return label
...
def main():
    ...
    print("\n=== Pairwise odds ratios (rows[0] vs rows[1], depleted vs not) ===")
    or_rows = []
    for g1, g2 in combinations(groups, 2):
        ...
        c, n2 = counts[g2]
        d = n2 - c
        or_est, lo, hi, p = odds_ratio_with_ci(a, b, c, d)
        tier = oddspath_tier(or_est)
        print(f"{g1} vs {g2}: OR={or_est:.1f} (95% CI {lo:.1f}-{hi:.1f}), "
              f"p={p:.2e}  -> OddsPath tier (point estimate): {tier}")
        or_rows.append({"group_a": g1, "group_b": g2, "OR": or_est,
                         "ci_lo": lo, "ci_hi": hi, "fisher_p": p,
                         "oddspath_tier_point_estimate": tier})
